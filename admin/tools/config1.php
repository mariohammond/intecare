...
function GetPositions($id, $period) {
    $sql = "SELECT count(a.PositionTitle) as `cnt`, a.PositionTitle, b.positionName FROM `agency_data` a
			INNER JOIN `positions` b ON b.positionId = a.PositionTitle WHERE a.InteCareAgencyID = " . $id . " AND a.Period = '" . $period . "'
...
function GetEmployeesForExport($id, $agency, $period) {
    ...
    $sql = "SELECT DISTINCT a.LastName, a.FirstName, a.PositionID, b.* FROM `agency_employees` a LEFT JOIN `agency_data` b on b.IdNumber = a.IdNumber
...
function GetGAActiveCount($agency) {
    ...
    $sql = "SELECT `GA_Active` FROM `agencies`  
...
function UpdateEmployees($active, $inactive, $id) {
	...
	`GA_Active`=" . $active . ",
	`GA_Inactive`=" . $inactive . " WHERE `AgencyId` = '" . $id . "'";
...
function GetRosterStatus() {
    ...
    $sql = "SELECT `value` FROM `options` WHERE `optionId` = 3";
    $result = $conn->query($sql);
