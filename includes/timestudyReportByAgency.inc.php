<?php
    ...
    foreach ($reportData as $data) {
        ...
        // Employee Name
        $firstNameInfo = $data[4];
        $fullName = $data[3] . ", $firstNameInfo";
        ...
        $replacements = array(0 => $endDate, 3 => $fullName, 6 => $reportDay);
        $tempData = array_replace($data, $replacements);

        // Code and Desc
        $sql1 = "SELECT log_code, log_desc FROM employee_timestudy WHERE mhfrpid = ? AND log_day = ?";
        $stmt1 = mysqli_stmt_init($conn);
        mysqli_stmt_prepare($stmt1, $sql1);
        mysqli_stmt_bind_param($stmt1, "is", $data[5], $reportDayInfo);
    ...
    $xls_filename = 'INFinalResultsByAgency_' . $reportData[0][0] . '.xls';
